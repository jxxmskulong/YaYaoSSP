...
public class CatchRecordServiceImpl implements CatchRecordService{
	...
	@Override
	public int countAll(Integer goodsId,Integer acountId,Integer status) {
		int c = catchRecordDao.countAll( goodsId,acountId,status);
		return c;
	}

	@Override
	public List<CatchRecord> browsePagingCatchRecord(Integer goodsId,Integer acountId,Integer status,int pageNum, int pageSize,
		...
		List<CatchRecord> l = catchRecordDao.browsePagingCatchRecord(goodsId, acountId,status,pageNum-1, pageSize, orderName, orderWay);
		return l;
	}
